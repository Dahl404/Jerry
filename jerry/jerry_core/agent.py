...
class Agent:
    ...
    def __init__(self, state: State, executor: Executor, tui=None):
        ...
        self.conv:    List[Dict] = [{"role": "system", "content": SYSTEM_PROMPT.format(jerry_base=JERRY_BASE)}]
        self._stop    = False
        ...
        while not self._stop:
            try:
                ...
                if pending:
                    ...
                    # Send [continue] token to keep working (no verbose task prompts)
                    if current_task != last_task_text:
                        last_task_text = current_task
                        # Just signal to continue working
    ...
    def _cycle(self, max_turns: int = 30):
        ...
        try:
            for turn in range(max_turns):
                ...
                # IMPORTANT: When tool calls exist, don't save content to prevent 400 errors
                # The text was already streamed to user, so it's not lost
                self.conv.append({
                    "role":       "assistant",
                    "content":    None if tool_calls else msg.get("content"),
    ...
    def _check_tool_loop(self, tool_calls: List[Dict], recent: List[str],
        ...
        for tc in tool_calls:
            ...
            if len(recent) >= max_repeats and len(set(recent)) == 1:
                ...
                self.state.push_chat("jerry", f"⚠️ I got stuck repeating `{name}`. Let me try a different approach.")
                self._is_thinking = False
                self.state.set_status("ready")
                return True
        return False
                    ...
                    if line_str.startswith('data: '):
                        ...
                        try:
                            ...
                            if delta.get('reasoning_content'):
                                ...
                                self.state.push_chat("jerry", thinking_content, expression="thinking", replace_last=True)
                                # Set thinking face
                                try:
                            ...
                            if delta.get('content'):
                                ...
                                self.state.push_chat("jerry", response_content, replace_last=True)
