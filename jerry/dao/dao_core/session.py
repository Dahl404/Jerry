#!/usr/bin/env python3
"""Dao — Session Manager for Shutdown, Summaries, and Archival"""

import os
import json
from datetime import datetime
from .models import State
from .config import SUMMARY_DIR, LOGS_DIR, PERSONA_DIR, DAO_BASE
...
class SessionManager:
    ...
    def on_shutdown(self):
        """Called when Dao is shutting down. Compresses context, saves persona, archives logs."""
    ...
    def _save_summary(self, timestamp: str):
        ...
        summary_lines = []
        summary_lines.append(f"=== Dao Session Summary ===")
        ...
        if chat:
            ...
            dao_msgs = [m for m in chat if m.role == "dao"]
            summary_lines.append(f"  User messages: {len(user_msgs)}")
            summary_lines.append(f"  Dao messages: {len(dao_msgs)}")
            summary_lines.append("")
    ...
    def _save_persona(self, timestamp: str):
        ...
        persona_lines = []
        persona_lines.append(f"=== Dao Persona Document ===")
        ...
        dao_msgs = [m for m in chat if m.role == "dao"]
        if dao_msgs:
            avg_len = sum(len(m.text) for m in dao_msgs) / len(dao_msgs)
