...
class Executor:
    ...
    def _dispatch(self, name: str, a: Dict) -> str:
        ...
        elif name == "todo_add":             return self._tadd(a.get("task"), a.get("tasks"), a.get("priority", "medium"))
        elif name == "todo_complete":        return self._tdone(a.get("index"), a.get("id"))
            ...
            try:
                ...
                if streamer:
                    ...
                    if screen and len(screen) > 50 and not screen.startswith("ERROR"):
                        # Return screen preview so agent sees what happened
                        return f"✓ Keys processed\n\n[Screen after keypress:]\n{screen[:500]}"
            except Exception:
                pass
    ...
    def _run_program(self, command: str, session: str = "dao-control") -> str:
        ...
        Args:
            command: Command to run (e.g., 'python script.py', 'nvim file.txt')
            session: tmux session name (default: 'dao-control')
        """
        try:
            # Enable stream mode in state (TUI will check this)
            self.state.enable_stream_mode(session)

            # Set target session for terminal controller
            controller = get_controller()
            controller.tmux_session = session

            # Start screen capture - runs command immediately in workspace
            # Use TUI's set_current_screen callback so TUI reads the same place executor writes
