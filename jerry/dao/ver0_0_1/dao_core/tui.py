...
Supports screen stream mode for watching Dao control other terminals.
...
THEME_DARK: Dict[str, Tuple[int, int]] = {
    ...
    "dao_lbl":   (curses.COLOR_GREEN,   -1),
    "dao_txt":   (curses.COLOR_WHITE,   -1),
...
class TUI:
        ...
        try:
            ...
            if self.face_enabled:
                ...
                if H < 60 or W < 100:
                    ...
                    try:
                        self.stdscr.addstr(0, 0, f"Terminal too small! Need 100x60, have {W}x{H}".center(W)[:W])
                        self.stdscr.addstr(2, 0, "Please resize your terminal window.".center(W)[:W])
                        self.stdscr.addstr(4, 0, f"Face panel needs 100 chars width.".center(W)[:W])
                        self.stdscr.addstr(6, 0, f"Or use: /face hide".center(W)[:W])
                        self.stdscr.refresh()
                    except curses.error:
                        pass
                    return
    ...
    def _draw_ai_speech(self, y: int, W: int):
        ...
        # Get last chat message from dao
        chat = self.state.chat[:]
        last_dao_msg = ""
        for msg in reversed(chat):
            if msg.role == "dao" and msg.text.strip():
                last_dao_msg = msg.text
                break
        
        if last_dao_msg:
            # Truncate to fit one line
            truncated = last_dao_msg[:W-4].replace('\n', ' ')
            try:
        ...
        try:
            ...
            for i, line in enumerate(face_lines[:h-2]):
                try:
                    ...
                    # Face content
                    self.stdscr.addstr(y + i + 1, x + 1, line[:w-2], curses.color_pair(_C["dao_txt"]))
                except curses.error:
                    pass
    ...
    def _draw_chat_feed(self, log: List[LogEntry], chat: List[ChatMsg],
        ...
        all_lines = []
        for msg in chat:
            role_label = "you: " if msg.role == "user" else "dao: "
        ...
        for i in range(ih):
            ...
            if is_thinking:
                ...
                cp = _C["user_txt"] if role == "user" else _C["dao_txt"]
                lbl = _C["user_lbl"] if role == "user" else _C["dao_lbl"]
            ...
            try:
                ...
                if role == "user":
                    ...
                    self.stdscr.addstr(y + 1 + i, x + 2, "dao: ",
                                       curses.color_pair(lbl) | curses.A_BOLD)
                    self.stdscr.addstr(y + 1 + i, x + 7, text[5:][:iw-7],
                                       curses.color_pair(cp))
            except curses.error:
    ...
    def _handle_command(self, raw: str) -> bool:
        ...
        if cmd in ("q", "quit", "exit"):
            ...
            if len(parts) > 1:
                ...
                # Start screen capture in dao_workspace
                try:
                    from .screen_stream import start_screen_stream
                    from .config import DAO_BASE
                    start_screen_stream(session, self.update_screen, auto_create=True, workdir=DAO_BASE)
