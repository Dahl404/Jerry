...
class Particle:
    def __init__(self, src_row, src_col, char):
        ...
        for col_idx, char in enumerate(line):
            if char not in (' ', '\n', '\t'):
                for _ in range(2):  # Double particles for a denser effect
                    ...
                    p = Particle(row_idx + off_r, col_idx + off_c, char)
                    particles.append(p)
    return particles
    ...
    for p in particles:
        ...
        if 0 <= py < height and 0 <= px < width - 1:
            ...
            if fading_out:
                ...
                if is_shimmering:
                    # Pulsing sine wave mapped to current time and particle's unique phase
...
def animate_scroll(stdscr, particles, height, width, splash_h, scale):
    ...
    if max_scroll <= 0:
        # If it all fits on screen, just shimmer in place for a bit longer
        hold_start = time.time()
    ...
    while True:
        ...
        render(stdscr, particles, height, width, current_time=time.time(), is_shimmering=True)
        time.sleep(1/fps)
    ...
    # Pause at the bottom with shimmer
    hold_start = time.time()
    while time.time() - hold_start < 2.0:
        render(stdscr, particles, height, width, time.time(), is_shimmering=True)
        time.sleep(1/fps)
...
def animate_out(stdscr, particles, height, width, jerry_frame=None):
    """Animate splash transitioning to Jerry UI by randomly flipping characters.
    
    Randomly writes Jerry's ASCII characters one at a time over the splash screen.
    Jerry frame is pre-captured and passed in.
    
    Args:
        stdscr: Curses screen
        particles: Particle array (unused)
        height: Screen height
        width: Screen width
        jerry_frame: Pre-captured Jerry UI frame (list of strings)
    """
    fps = 24
    duration = 1.5
    start_time = time.time()
    ...
    if not jerry_frame:
        ...
        jerry_frame = [[' ' for _ in range(width - 1)] for _ in range(height)]
    
    # Build list of all positions and shuffle for random order
    import random
    flip_positions = [(y, x) for y in range(height) for x in range(width - 1)]
    random.shuffle(flip_positions)
    
    total_positions = len(flip_positions)
    last_flipped = 0
    ...
    while True:
        ...
        t = elapsed / duration
        target_flipped = int(t * total_positions)
        
        # Write Jerry's characters at new random positions
        for i in range(last_flipped, target_flipped):
            if i < len(flip_positions):
                y, x = flip_positions[i]
                try:
                    stdscr.addch(y, x, jerry_frame[y][x], curses.A_NORMAL)
                except curses.error:
                    pass
        
        last_flipped = target_flipped
        stdscr.refresh()
        time.sleep(1/fps)
...
def main(stdscr, jerry_frame=None):
